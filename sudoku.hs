...
readGrid s
    ...
    | otherwise = Nothing
    where readCell '.' = Just $ Possible [1..9]
          readCell c
...
subGridsToRows = concatMap (\rows -> let [r1, r2, r3] = map (Data.List.Split.chunksOf 3) rows in zipWith3 (\a b c -> a ++ b ++ c) r1 r2 r3) . Data.List.Split.chunksOf 3
...
pruneGrid = fixM pruneGrid'
            where fixM f = f x >>= \x' -> if x' == x then return x else fixM f x'

nextGrids :: Grid -> (Grid, Grid)
nextGrids grid = let (i, first@(Fixed _), rest) = 
                         fixCell . Data.List.minimumBy (compare `Data.Function.on` (possibilityCount . snd))
                         . filter (isPossible . snd) . zip [0..] . concat $ grid
                 ...
                 where
                     ...
                     replace2D :: Int -> a -> [[a]] -> [[a]]
                     replace2D i v = let (x, y) = (i `quot` 9, i `mod` 9)
                                     in replace x (replace y (const v))
                                     where replace p f xs = [if i == p then f x else x | (x, i) <- zip xs [0..]]

isGridFilled :: Grid -> Bool
isGridFilled grid = null [ () | Possible _ <- concat grid]

isGridInvalid :: Grid -> Bool
isGridInvalid grid = any isRowInvalid grid || any isRowInvalid (Data.List.transpose grid) || any isRowInvalid (subGridsToRows grid)
    where isRowInvalid row = let fixeds = [x | Fixed x <- row]
                                 emptyPossibles = [x | Possible x <- row, null x]
                             in hasDups fixeds || not (null emptyPossibles)
                             where hasDups l = hasDups' l []
                                   hasDups' [] _ = False
                                   hasDups' (y:ys) xs
                                    | y `elem` xs = True
...
main = do
    ...
    Control.Monad.forM_ inputs $
        \input -> case readGrid input of
